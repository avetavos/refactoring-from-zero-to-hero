...
Contrast this with **[Replace Type Code with Subclasses](/refactoring/en/organizing-data/replace-type-code-with-subclasses/)**: that move bakes the variant into the object's *type*, which is perfect when the type is **fixed for life** (an employee's role) but impossible when it must change. The cost of State/Strategy is one more object and a layer of delegation; you pay it to gain a runtime-swappable, guardable lifecycle. If the type never changes, subclasses are simpler.

## Related

- [Replace Type Code with Subclasses](/refactoring/en/organizing-data/replace-type-code-with-subclasses/)
- [Encapsulate Variable](/refactoring/en/organizing-data/encapsulate-variable/)
- [Replace Primitive with Object](/refactoring/en/organizing-data/replace-primitive-with-object/)
