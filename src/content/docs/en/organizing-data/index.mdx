...
Five refactorings, each one tightening the relationship between data and the code that uses it:
...
- **Replace Type Code with Subclasses** — when a type-code field drives `switch`-style branching, replace the conditional logic with polymorphism across a small class hierarchy.
